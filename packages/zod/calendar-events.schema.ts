...
export const CalendarEventTypeSchema = z
  .enum(["APPOINTMENT", "BLOCK", "VACATION", "BREAK", "CUSTOM"])
...
export const CreateCalendarEventSchema = baseCalendarEventSchema.transform(
  (data, ctx) => {
...
export const UpdateCalendarEventSchema = baseCalendarEventSchema
  ...
  .transform((data, ctx) => {
    if (data.startTime && data.endTime) {
      const validationResult = validateCalendarEvent({
        ...data,
      } as CalendarEvent);
      if (validationResult !== true) {
        ctx.addIssue({
...
export const FindEventsParamsSchema = z
  .object({
